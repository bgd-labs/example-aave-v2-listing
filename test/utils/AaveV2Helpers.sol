...
library AaveV2Helpers {
  ...
  {
    AaveAddressBookV2.Market memory market = AaveAddressBookV2.getMarket(
  ...
  function _getStructReserveConfig(
    ...
    ConfigLite memory configData = _getConfigData(
      marketName,
  ...
  function _getConfigData(string memory marketName, address tokenAddress)
    internal
    view
    returns (ConfigLite memory)
  {
    AaveAddressBookV2.Market memory market = AaveAddressBookV2.getMarket(
      marketName
    );
    (
      ...
      bool isActive,
      bool isFrozen
    ) = market.AAVE_PROTOCOL_DATA_PROVIDER.getReserveConfigurationData(
        tokenAddress
      );
    address interestRateStrategyAddress = market
      .POOL
  ...
  function _getStructReserveTokens(
    ...
    ) = market.AAVE_PROTOCOL_DATA_PROVIDER.getReserveTokensAddresses(
      underlyingAddress
    );
